...
mkTracers blockConfig tOpts@(TracingOn trSel) tr nodeKern = do
  ...
  pure Tracers
    { chainDBTracer = tracerOnOff' (traceChainDB trSel) $
        annotateSeverity $ teeTraceChainTip blockConfig tOpts fStats elidedChainDB
...
teeTraceChainTip _ TracingOff _ _ _ _ = nullTracer
teeTraceChainTip blockConfig (TracingOn trSel) fStats elided trTrc trMet =
  Tracer $ \ev -> do
    traceWith (teeTraceChainTipElide (traceVerbosity trSel) elided trTrc) ev
    traceChainMetrics' <- traceChainMetrics blockConfig fStats trMet
...
traceChainMetrics blockConfig fStats tr = do
  ...
  return . Tracer $ \ev ->
    fromMaybe (pure ()) $
      doTrace tBlocksUncoupled <$> chainTipInformation ev
  where
